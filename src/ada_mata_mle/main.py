...
def train(config: str = typer.Option("settings.yaml", help="Path to config file")):
    ...
    print(f"   Model: {cfg['train']['model_type']} | Epochs: {cfg['train']['epochs']} | ImgSize: {cfg['dataset']['img_size']}")
    ...
    print(f"✅ Training Selesai! Hasil tersimpan di runs/train/{cfg['experiment_name']}")
    
    # Auto-export ke ONNX setelah training selesai (biar siap inference cepat)
    print("📦 Meng-export model terbaik ke ONNX...")
    model.export(format='onnx', imgsz=cfg['dataset']['img_size'], dynamic=False, opset=12)
...
def infer(
    ...
    if not weights_path:
        ...
        print("   Pastikan kamu sudah menjalankan training (bsort train) atau notebook eksperimen.")
        sys.exit(1)
